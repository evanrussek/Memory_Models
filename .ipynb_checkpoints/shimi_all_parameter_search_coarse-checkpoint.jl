...
if on_cluster
    to_save_folder = "/home/erussek/projects/Memory_Models/shimi_all_parameter_search_coarse"
else
    to_save_folder = "/Users/erussek/Dropbox/Griffiths_Lab_Stuff/code/Memory_Models/shimi_all_parameter_search_coarse"
end
...
NT_vals = [12, 25, 50, 100, 200, 400, 800, 1600, 3200] # run w these now... 
...
n_jobs_per_run = 11 # should be about 30 minutes...
...
for this_job_idx in these_jobs
    ...
    local job_res_1 = sim_exp1(epsilon, N_Quanta, NT_per_Second; mem_slope = mem_slope, return_last_only=false, N_Trials = N_Trials);
    local full_file_path = joinpath(to_save_folder,"exp1",file_name)
    jldsave(full_file_path; job_res_1)

    local job_res_2 = sim_exp2(epsilon, N_Quanta, NT_per_Second; mem_slope = mem_slope,return_last_only=false, N_Trials = N_Trials);
    local full_file_path = joinpath(to_save_folder,"exp2",file_name)
    jldsave(full_file_path; job_res_2)
    
    local job_res_3 = sim_exp3(epsilon, N_Quanta, NT_per_Second; mem_slope = mem_slope, return_last_only=false, N_Trials = N_Trials);
